...
# Title
st.title(':green[Welcome to Youtube Data Harvesting Mini Project]')
...
with col1:
    st.header(':blue[Find data by YT channel ID]')
    channel_id = st.text_input('**_Enter Youtube channel id_**')
    st.write('''If you want to store above YT channel id data into your MongoDB, Please click on **:orange['Get data and stored']**.''')
    ...
    if Get_data or st.session_state.Get_state:
            ...
            try:
                try:
                    ...
                    if 'items' not in channel_response:
                        ...
                        st.error("Enter the correct _**channel_id**_")
                        return None
        ...
        uri = #your Atlas mongodb server url
        ...
        # create a collection
        coll_name = " " #your collection name
...
# ***********************************************   Stored data to MySQL    ***********************************************

with col2:
    st.header(':blue[Store data in MySQL]')
    ...
    st.write('''To Store mongoDB data in MySQL server Select channel name in above box and please click on below **:orange['Store data in MySQL']**.''')
    Migrate = st.button('**Store data in MySQL**')
    ...
    if Migrate or st.session_state_migrate_sql:
        ...
        #Login to "freesqldatabase.com" and Use credentials to use online sql server
        ...
        mycursor.execute("CREATE DATABASE IF NOT EXISTS --enter database_name--")
        ...
        # fill below parameters like username, password, host, database_name
        engine = create_engine('mysql+mysqlconnector://username:password@host/database_name', echo=False)
...
# ************************************    Questions     ************************************************ 
st.subheader(':blue[ Quick Channels Analysis ]')

# Selectbox creation
question_tosql = st.selectbox('_**Select your Question**_',
...
connect_for_question = pymysql.connect(host='', user='', password='', db='')
